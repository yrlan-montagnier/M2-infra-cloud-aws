...
resource "aws_launch_template" "nextcloud" {
  ...
  # Associer le profil d'instance IAM
...
# Récupérer les adresses IP privées des instances créées par l'ASG
...
# Output pour afficher les adresses IP privées des instances créées par l'ASG
